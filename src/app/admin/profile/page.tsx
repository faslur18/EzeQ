...
export default function AdminProfileWizardPage() {
    ...
    const isSalonAdmin = (session?.user as { role?: string } | undefined)?.role === "SALON_ADMIN"

    if (status === "unauthenticated") {
        router.push("/auth/login")
    }

    if (status === "authenticated" && !isSalonAdmin) {
        ...
        try {
            const payload = {
                basicIdentity: {
                    ...
                    shortDescription,
                    logoImageUrl,
                    coverImageUrls: coverImageUrls.filter(Boolean),
                },
                locationContact: {
                    fullAddress,
                    landmark,
